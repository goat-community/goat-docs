...
**Gravity-based Heatmaps** use a color-coded map to visualize the accessibility of [POIs](../../further_reading/glossary#point-of-interest-poi "What is a POI?") and point-based data.

## 1. Explanation

Visualized as a color-coded hexagonal grid, heatmaps take into account real-world transport and street networks to compute accessibility. When you specify a routing mode (such as `walking`, `bicycle`, etc.) and a maximum travel duration, a grid reflecting all areas accessible under these conditions is produced.
...
Described succinctly, heatmaps are a visualization representing *access* from various unspecified origins to one or more specified destinations. This is in contrast to catchment areas which represent *egress* from one or more specified origins to various unspecified destinations.

:::

TODO: Update this image

![Catchment Area in GOAT](/img/toolbox/accessibility_indicators/catchments/catchment_sample.png "Gravity-based Heatmap in GOAT")
  
:::info 
The calculation of heatmaps is currently possible in specific regions. Upon selecting a routing mode, you will see a **geofence** on the map indicating supported regions.

TODO: Update this image to reflect the latest geofence for heatmaps
...
TODO: Add image of impedance function

Travel times are computed in minutes. If a max traveltime of 30 minutes is used, this means that destinations which are further than 30 minutes are considered as non accessible and thus not considered in the calculation of the accessibility. The *sensitivity* parameter defines how accessibility changes with increasing travel time. As the *sensitivity* parameter is decisive when measuring accessibility, GOAT allows you to adjust them. The following graphs show the influence of the *sensitivity* parameter on accessibility.
